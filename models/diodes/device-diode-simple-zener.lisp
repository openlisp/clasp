...
(defmethod map-device ((d class-diode-simple-zener) (m matrice-system))
  ...
  (let ((v+       (make-var-node 'v (node+ d)))
    ...
    (set-model-value m 'nonlinear-equation-vector i (diode-simple-zener-current d v+ v-))
    
    ;        (set-d-value m i  v+  (diode-simple-zener-current-dv+ d v+ v-))
    ;        (set-d-value m i  v-  (diode-simple-zener-current-dv- d v+ v-))
    
    (set-model-value m 'differetial-equation-array i v+ (diode-simple-zener-current-dv+ d v+ v-))
    (set-model-value m 'differetial-equation-array i v- (diode-simple-zener-current-dv- d v+ v-))
    ...
    (set-model-value m 'initial-number-vector i 0.05d0)
